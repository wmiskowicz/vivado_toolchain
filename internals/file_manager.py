...
class FileManager:
  ...
  def prioritize_pkg_if(self, file_list):
      """
  ...
  def update_project_details_tcl(self):
      ...
      xdc_abs = self.collect_files_abs(FPGA_CONSTRAINTS_DIR, {".xdc"})
      xci_abs, sv_abs, v_abs, vhdl_abs = [], [], [], []

      sv_abs   += self.collect_files_abs(FPGA_RTL_DIR, {".sv"})
      v_abs    += self.collect_files_abs(FPGA_RTL_DIR, {".v"})
      vhdl_abs += self.collect_files_abs(FPGA_RTL_DIR, {".vhd", ".vhdl"})

      sv_abs   += self.collect_files_abs(TOP_RTL_DIR,  {".sv"})
      v_abs    += self.collect_files_abs(TOP_RTL_DIR,  {".v"})
      vhdl_abs += self.collect_files_abs(TOP_RTL_DIR,  {".vhd", ".vhdl"})

      xci_abs += self.collect_files_abs(TOP_RTL_DIR, {".xci"})

      # Collect memory .data files
      mem_abs = self.collect_files_abs(MEM_INIT_DIR, {".data"})
      ...
      sv_files   = self.prioritize_pkg_if(to_rel_fpga(sv_abs))
...
"""
      ...
      print(colorama.Fore.GREEN + f"[INFO] Updated {OUTPUT_TCL} with new file lists.")
